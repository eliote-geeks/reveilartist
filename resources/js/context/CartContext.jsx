...
export const CartProvider = ({ children }) => {
    ...
    const addToCart = (item) => {
        ...
        if (existingItem) {
            // Pour les sons, ne pas permettre l'ajout multiple
            if (item.type === 'sound' || !item.type) {
                toast.warning('Déjà dans le panier', `"${item.title}" est déjà dans votre panier`);
                return false;
            }

            // Pour les autres types (événements), incrémenter la quantité
