...
class UserController extends Controller
{
    ...
    public function getFavoriteSounds(Request $request)
    {
        try {
            ...
            $favoriteSounds = $user->favoriteSounds()
                ...
                ->where('status', 'published')
                ->latest('favorite_sounds.created_at')
                ->get()
                ->map(function ($sound) {
                    return [
                        'id' => $sound->id,
    ...
    public function getCompleteProfile(Request $request)
    {
        try {
            ...
            // Charger les sons de l'utilisateur avec pagination
            ...
            $stats = [
                'sounds' => [
                    'total' => $sounds->total(),
                    'free' => Sound::where('user_id', $user->id)->where('is_free', true)->count(),
                    'paid' => Sound::where('user_id', $user->id)->where('is_free', false)->count(),
                    'plays' => Sound::where('user_id', $user->id)->sum('plays_count'),
                    'downloads' => Sound::where('user_id', $user->id)->sum('downloads_count'),
                    'revenue' => Sound::where('user_id', $user->id)->where('is_free', false)->sum('price')
                ],
                'events' => [
                    'total' => $events->count(),
