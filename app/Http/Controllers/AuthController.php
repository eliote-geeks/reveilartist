...
class AuthController extends Controller
{
    ...
    public function getCompleteProfile(Request $request)
    {
        try {
            ...
            $stats = [
                'sounds_purchased' => $user->purchasedSounds()->count(),
                'events_attended' => $user->purchasedEvents()->count(),
                'sounds_created' => $user->sounds()->count(),
                'events_organized' => $user->events()->count(),
                'following_count' => $user->following()->count(),
                'followers_count' => $user->followers()->count(),
                'total_spent' => $user->payments()->where('type', 'purchase')->sum('amount'),
                'total_earned' => $user->payments()->where('type', 'sale')->sum('amount'),
                'total_plays' => $user->sounds()->sum('plays_count'),
                'total_downloads' => $user->sounds()->sum('downloads_count'),
                'total_likes' => $user->sounds()->sum('likes_count'),
            ];
            ...
            return response()->json([
                ...
                'user_sounds' => $userSounds,
                'user_events' => $userEvents,
                'following' => $following,
                'followers' => $followers,
                'stats' => $stats,
                'meta' => [
                    'current_page' => $purchasedSounds->currentPage(),
                    'last_page' => $purchasedSounds->lastPage(),
                    'per_page' => $purchasedSounds->perPage(),
                    'total' => $purchasedSounds->total()
                ]
            ]);
